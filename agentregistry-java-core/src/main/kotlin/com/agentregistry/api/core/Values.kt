...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AgentregistryInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AgentregistryInvalidDataException("`$name` is not set")
            is JsonNull -> throw AgentregistryInvalidDataException("`$name` is null")
            else -> throw AgentregistryInvalidDataException("`$name` is invalid, received $this")
        }
